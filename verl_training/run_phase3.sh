#!/usr/bin/env bash
# Phase 3: Full rewards (accuracy + tag_count + format) with speculative reasoning
#
# Same as Phase 2 but with SPLITREASON_FULL_REWARDS=1 to enable weighted
# reward combination and component logging.
#
# GPU allocation:
...
#   4. Launch the 32B big model server (see run_phase2.sh header)
#
# Usage:
#   bash verl_training/run_phase3.sh
...
# Enable full rewards (Phase 3)
export SPLITREASON_FULL_REWARDS=1
...
if ! curl -s "http://${SPLITREASON_BIG_MODEL_HOST}:${SPLITREASON_BIG_MODEL_PORT}/v1/models" > /dev/null 2>&1; then
    ...
    echo "Make sure to launch the 32B vLLM server first."
fi
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=phase3_full_rewards \
    trainer.rollout_data_dir=outputs/phase3_generations
