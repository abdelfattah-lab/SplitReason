...
def run_inverted_scaling_law_plot_and_random_ablation_plot(
    csv_path: str = "random_offload_ablation.csv",
    coverage_start: float = 0.00,
    coverage_end: float = 0.30,
    coverage_step: float = 0.01,
    sims_per_coverage: int = 1,
    p_t: int = 2048,
    d_t: int = 16384,
    outfile: str = "random_ablation_and_inverted_scaling.pdf",
):
    """
    Make a 21×7″ two‑panel figure:
      • **Left**  – accuracy box‑and‑whisker for the random‑offload ablation CSV
      • **Right** – the “inverted” scaling‑law plot (fraction small‑model vs speed‑up)

    Depends on `simulate_inference_time` already being in the namespace.
    """
    import pandas as pd
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # ----------  figure scaffold ----------
    fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=(21, 7))

    # ----------  LEFT: random‑offload ablation ----------
    ...
    df["condition"] = (
        df["offload_type"]
        .astype(str)
        .replace({"random": "Random", "learned": "Learned", "none": "None"})
        + "\n("
        + df["offload_percent"].astype(str)
        + "%)"
    )
    df["accuracy"] *= 100.0  # convert to %

    # -------- set the order so any “Learned …” label is last --------
    all_labels   = list(df["condition"].unique())
    learned_lbls = [lbl for lbl in all_labels if "Learned" in lbl]
    other_lbls   = [lbl for lbl in all_labels if "Learned" not in lbl]
    ordered_lbls = other_lbls + learned_lbls          # learned goes last

    df["condition"] = pd.Categorical(
        df["condition"],
        categories=ordered_lbls,
        ordered=True
    )


    # df.boxplot(column="accuracy", by="condition", ax=ax_left)
    df.boxplot(
        column="accuracy",
        by="condition",
        ax=ax_left,
    ...
    ax_left.set_ylabel("Accuracy (%)", fontsize=22)
    # increase fontsize of x/y ticks
    ax_left.tick_params(axis='x', labelsize=18)
    ax_left.tick_params(axis='y', labelsize=18)
    # ax_left.grid(False, axis="x", linestyle=":", linewidth=0.5, alpha=0.7)
    ax_left.grid(False)
    fig.suptitle("")  # drop pandas’ default super‑title

    # ----------  RIGHT: inverted scaling‑law simulation ----------
    # Prefill (tok/s) and decode (tok/s) rates for each model
    prefill_map = {
        "1.5B": 28868.4664,
        "7B":   7658.083194,
        "8B":   7456.973294,
    ...
    cmap   = plt.cm.CMRmap
    colors = cmap(np.linspace(0, 1, int(1.5 * len(big_models))))

    for color, big_model in zip(colors, big_models):
        big_pre, big_dec = prefill_map[big_model], decode_map[big_model]
        big_only_time    = (p_t / big_pre) + (d_t / big_dec)

        x_speedup, y_frac_small = [], []

        for cov in coverage_values:        # cov == fraction done by BIG model
            sim_times = []
            for _ in range(sims_per_coverage):
                sim = simulate_inference_time(
                    p_t=p_t,
                    d_t=d_t,
                    big_pre=big_pre,
                    big_dec=big_dec,
                ...
                sim_times.append(sim["total_time_s"])

            mean_time   = np.mean(sim_times)
            speedup_vs_big = big_only_time / mean_time

            x_speedup.append(speedup_vs_big)
            y_frac_small.append(1.0 - cov)   # fraction generated by the *small* model
    ...
    ax_right.set_title("Two‑Model Pipelined Inference Simulation\nFraction Decoded By Small‑Model vs Speed‑up", fontsize=22)
    ax_right.set_xlabel("Speed‑up over Big Model", fontsize=22)
    ax_right.set_ylabel("Fraction Generated by Small Model", fontsize=22)
    ax_right.tick_params(axis='x', labelsize=18)
    ax_right.tick_params(axis='y', labelsize=18)
    # x axis log
    # ax_right.set_xscale('log')
    ax_right.grid(True)
    ax_right.legend(fontsize=22)
...
def parse_args():
    ...
    parser.add_argument('--big_coverage', type=float, default=0.0778, help='Fraction of decode tokens covered by the big model')
...
if __name__ == "__main__":
    ...
    simulations = []
    for _ in range(10):
        # For now, this simulation doesnt really need repetitions. May be important later.
        sim_result = simulate_inference_time(
            p_t=p_t,
            d_t=d_t,
            big_pre=big_pre,
    ...
    run_inverted_scaling_law_plot_and_random_ablation_plot()