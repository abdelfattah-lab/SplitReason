...
export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=120000  # 20 min, adjust as needed
export NCCL_P2P_DISABLE=1
export TORCH_DISTRIBUTED_DEFAULT_TIMEOUT=216000  # 6 h collective timeout
CUDA_VISIBLE_DEVICES=0,1,2,3 ACCELERATE_LOG_LEVEL=info \
  accelerate launch --main_process_port 29507 --config_file recipes/accelerate_configs/zero2.yaml --num_processes 4 \
  src/open_r1/grpo.py --config recipes/DeepSeek-R1-Distill-Qwen-1.5B/grpo/config_demo_14b.yaml \
  --wandb_project SpeculativeReasoning --run_name E2EGRPO_bm14B_64Gen_4K_2xAcc
